...
BC1 <- read.table("../Trasferv6/D1_4Cl.tsv",sep="\t",header=TRUE)
BC2 <- read.table("../Trasferv6/D2_4Cl.tsv",sep="\t",header=TRUE)
BC3 <- read.table("../Trasferv6/D3_6Cl.tsv",sep="\t",header=TRUE)
BC3B <- read.table("../Trasferv6/D3B_6Cl.tsv",sep="\t",header=TRUE)
BC6 <- read.table("../Trasferv6/D6_4Cl.tsv",sep="\t",header=TRUE)
BC7 <- read.table("../Trasferv6/D7_4Cl.tsv",sep="\t",header=TRUE)
BC8 <- read.table("../Trasferv6/D8_4Cl.tsv",sep="\t",header=TRUE)
...
Emb <- readRDS("../../Thorsten/Dylan/DylanHarmony/MagdaHumanDataHarmonyhg38_labelled.rds")
DefaultAssay(Emb) <- "RNA"

#Load in the endometrial data Wang et al.
...
#Assign endometrial data a matching "batch" to the embryo (there are 6 in total and from different stages). This is somewhat arbitrary but fine as we want to pull out general endometrial markers rather
#than stage specific ones at this point
Dep1$ID3 <- "B1"
Dep2$ID3 <- "B2"
Dep3$ID3 <- "B3"
Dep4$ID3 <- "B4"
...
Idents(D6,cells=BC6$barcode[which(BC6$assignment==4)]) <- "G5"
Idents(D6,cells=BC6$barcode[which(BC6$assignment==5)]) <- "G6"
...
mammal.anchors <- FindIntegrationAnchors(object.list = list(MergedData1,MergedData2,MergedData3,MergedData4,MergedData5,MergedData6,D1,D2,D3,D3B,D6,D7,D8), dims = 1:20, anchor.features = 4000)
