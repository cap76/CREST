...
#ID3 <- mammal.combined$ID3
#ID3[which(is.na(mammal.combined$ID3) ==1)] <- "M1"
#mammal.combined$ID3 <- ID3

#Redo plot and check everything is the same.
...
#Save cell idents to new slot (should already have been donne in last script but just in case)
...
DimPlot(mammal.combined, reduction = "pca", split.by = "ID3", label = TRUE, repel = TRUE)
ggsave(filename=paste(saveext,"PCA_Seurat-Cl10",".pdf",sep=""),width = 60, height = 10, limitsize = FALSE, useDingbats=FALSE)
DimPlot(mammal.combined, reduction = "umap", split.by = "ID3", label = TRUE, repel = TRUE)
ggsave(filename=paste(saveext,"UMAP_Seurat-Cl10",".pdf",sep=""),width = 60, height = 10, limitsize = FALSE, useDingbats=FALSE)
mammal.combined$Cl10 <- Idents(mammal.combined)

mammal.combined <- FindClusters(mammal.combined, resolution = 1.5)
DimPlot(mammal.combined, reduction = "umap", split.by = "ID3", label = TRUE, repel = TRUE)
ggsave(filename=paste(saveext,"UMAP_Seurat-Cl15",".pdf",sep=""),width = 60, height = 10, limitsize = FALSE, useDingbats=FALSE)
mammal.combined$Cl15 <- Idents(mammal.combined)

mammal.combined <- FindClusters(mammal.combined, resolution = 0.5)
DimPlot(mammal.combined, reduction = "pca", split.by = "ID3", label = TRUE, repel = TRUE)
ggsave(filename=paste(saveext,"PCA_Seurat-Cl05",".pdf",sep=""),width = 60, height = 10, limitsize = FALSE, useDingbats=FALSE)
DimPlot(mammal.combined, reduction = "umap", split.by = "ID3", label = TRUE, repel = TRUE)
ggsave(filename=paste(saveext,"UMAP_Seurat-Cl05",".pdf",sep=""),width = 60, height = 10, limitsize = FALSE, useDingbats=FALSE)
mammal.combined$Cl05 <- Idents(mammal.combined)


#Save the clustered data
saveRDS(mammal.combined, file = paste(saveext,"Seurat_combined2.rds",sep=""))
